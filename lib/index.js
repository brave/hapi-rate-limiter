...
exports.register = (server, options, next) => {
  ...
  server.ext('onPostAuth', (request, reply) => {
    ...
    if (!options.methods || options.methods.indexOf(request.route.method) === -1) {
      if (!(request.route.method === 'get' || request.route.method === 'post' || request.route.method === 'delete')) {
        return reply.continue();
      }
    }

    let routeSettings = request.route.settings.plugins.rateLimit;
    if (routeSettings ? !routeSettings.enabled : !options.enabled) {
      return reply.continue();
    }
    if (!routeSettings) {
      routeSettings = {};
